 ...
 * The bar itself.
 *
 * `portfolio` is the Real Estate menu, assembled from the developments
 * published in the dashboard and handed down by the server component in
 * `site-nav.tsx`. Everything else in the menu is fixed content. A nav that
 * reads the inventory cannot be rendered on the client alone, and a nav that
 * does not read it goes stale the first time somebody publishes a listing.
 */
export function SiteNavBar({ portfolio = [] }: { portfolio?: MenuCard[] }) {
  ...
  /*
   * Real Estate's cards come from what is published; the rest are written in
   * `content.ts`. A portfolio menu with nothing in it is not rendered at all —
   * the label stays a plain link rather than opening onto an empty panel.
   */
  const menuFor = (key: NavKey): MegaMenu | undefined => {
    const menu = menus[key];
    if (!menu || menu.kind !== "portfolio") return menu;
    const cards = menu.cards.length ? menu.cards : portfolio;
    return cards.length ? { ...menu, cards } : undefined;
  };

  const renderedMenu: MegaMenu | undefined =
    rendered && rendered !== LANGUAGE ? menuFor(rendered) : undefined;
      ...
      <div className="mx-auto flex w-full max-w-[1440px] items-center justify-between gap-8 px-6 py-3 sm:px-10 lg:px-[72px] lg:py-3.5">
        ...
        <nav className="hidden items-center gap-7 xl:flex">
          {navLinks.map((link) => {
            ...
            const hasMenu = link.hasMenu && Boolean(menuFor(link.key));
            return (
              <div
                key={link.key}
                className="relative"
                onMouseEnter={() =>
                  hasMenu ? openMenu(link.key) : scheduleClose()
                }
              >
                {hasMenu ? (
                  <button
      ...
      {open ? (
        <div className="max-h-[calc(100svh-72px)] overflow-y-auto border-t border-white/15 xl:hidden">
          <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-1 px-6 py-6 sm:px-10">
            {navLinks.map((link) => {
              const menu = link.hasMenu ? menuFor(link.key) : undefined;
